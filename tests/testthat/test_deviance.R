...
test_that("Testing attendance example", 
          { 
            ...
            expect_equal(round(AIC(M.attendance),3),1739.061)
            expect_equal(round(M.attendance$null.deviance,3), 456.388)
            expect_equal(round(M.attendance$residuals.deviance,3), 377.873)
          })
...
test_that("Testing takeover example",
          {
            ...
            M.bids <- glm.cmp(numbids ~ leglrest + rearest + finrest + whtknght 
                              ...
                              data=takeoverbids)
            expect_equal(round(AIC(M.bids),3), 382.178)
...
test_that("Testing cottonbolls example",
          {data(cottonbolls)
            ...
            expect_equal(round(AIC(M.bolls),3), 440.823)
...
test_that("Testing fish example",
          { data(fish)
            ...
            expect_equal(round(AIC(M.fish),3), 638.888)
