...
It appears students enrolled into the General program tend to miss more days of school than those enrolled into the Academic or the Vocotional program. There is some overdispersion in this dataset. We can use `glm.cmp()` to fit the mean-parametrized CMP model to try to account for that overdispersion. 
...
Please refer to `?gg_plot` to see what diagnostic plots are available. 
...
modelsummary::modelsummary(list(M.attendance, M_poisson, M_nb))
```
