...
#' @param ... other arguments passed to or from other methods  (currently unused).
#' @details
#' \code{print.summary.cmp} tries to be smart about formatting the coefficients, standard errors
...
summary.cmp <- function(object, ...) {
  if (object$const_nu) {
    ...
    parameter <- rep(
      c("mu", "nu"),
      c(
        length(object$coefficients_beta),
    ...
    coef.table_beta <-
      cbind(
        estimate_beta, std.error_beta, statistic_beta,
        p.value_beta
      )
    ...
    coef.table_gamma <-
      cbind(
        estimate_gamma, std.error_gamma, statistic_gamma,
        p.value_gamma
      )
    dimnames(coef.table_gamma) <-
      list(
        names(estimate_gamma),
        c("Estimate", "Std.Err", "Z value", "Pr(>|z|)")
      )
    coef.table <- cbind(parameter, rbind(
      coef.table_beta,
      coef.table_gamma
    ))
  }
  keep <- match(
    c(
      "call", "const_nu", "terms_mu", "terms_nu", "nu",
  ...
  if (!object$const_nu) {
    ans <- c(
      ans,
      list(
        coef.table_beta = coef.table_beta,
...
print.summary.cmp <- function(x, digits = max(3, getOption("digits") - 3),
  ...
  if (x$const_nu) {
    ...
    printCoefmat(x$coef.table_beta,
      digits = digits,
      signif.stars = signif.stars,
      na.print = "NA", ...
    )
  ...
  if (x$const_nu) {
    ...
    printCoefmat(x$coef.table_gamma,
      digits = digits,
      signif.stars = signif.stars,
      na.print = "NA", ...
    )
...
augment.cmp <- function(x, data = model.frame.cmp(x),
  ...
  if (se_fit) {
    pred_obj <- predict.cmp(x, newdata,
      type = type.predict,
      se.fit = TRUE
    )
